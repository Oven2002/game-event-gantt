...
// Implicit localStorage contract with the mascot widget (vendor/live2d-widget):
// when the widget reads a waifu-display timestamp less than 24 hours old, it
// shows only the recall toggle and defers loading the mascot; when
// waifu-disabled is "true" the widget is never auto-loaded at all
// (see initWidget in vendor/live2d-widget/src/widget.ts).
// "Shown" removes both keys so the widget resumes automatic loading.
// Recheck widget.ts before changing this logic.
