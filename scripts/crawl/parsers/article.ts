...
function classify(title: string): ArticleClassification | undefined {
  ...
  if (/版本/.test(title) && /活动/.test(title)) return { kind: "event", type: "event" };
  if (/(?:版本.*(?:更新|研发|说明|公告)|更新说明)/.test(title)) return { kind: "version" };
  if (/活动/.test(title)) return { kind: "event", type: "event" };
  return undefined;
}

function hasConcreteVersionIdentity(title: string, content: string): boolean {
  return /\d+(?:\.\d+)+\s*版本/.test(title)
    || /版本\s*\d+(?:\.\d+)+/.test(title)
...
export function parseArticleCandidates(article: { title: string; content: string; publishedAt?: string | null }): ParsedArticleCandidate {
  ...
  try {
    ...
    if (classification.kind === "event" && /维护(?:结束)?后(?:开启|开放|开始)/.test(article.content)) {
      ...
      return { ...base, status: "needs_review", reason: inferred.note ?? "maintenance inference needs review" };
    }
...
export function parseArticleCandidate(raw: RawArticle, runId: string, semanticSlot: string): CandidateItem {
  ...
  const candidate = parsed.status === "ready" && parsed.kind && parsed.start
    ? parsed.kind === "version"
      ? { ...base, kind: "version" as const, review: "ready" as const, start: parsed.start, end: parsed.end!, ...(parsed.note ? { note: parsed.note } : {}), timeCertainty: { start: parsed.certainty ?? "confirmed", end: "confirmed" as const } }
      : { ...base, kind: "event" as const, review: "ready" as const, type: parsed.type ?? "event", start: parsed.start, ...(parsed.end ? { end: parsed.end } : {}), ...(parsed.note ? { note: parsed.note } : {}), timeCertainty: { start: parsed.certainty ?? "confirmed", ...(parsed.end ? { end: "confirmed" as const } : {}) } }
